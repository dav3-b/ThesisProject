#!/bin/bash
#SBATCH --job-name=e3b_ckpt25_test               	# Job name
#SBATCH --partition=					# Specify the partition or queue name
#SBATCH --gres=gpu:1                        		# GPU
#SBATCH --mem=24G                        		# Memory per node (in GB)
#SBATCH --time=24:00:00                 		# Time limit (hh:mm:ss)
#SBATCH --output=e3b_17_ckpt25_test.out             	# Standard output file
#SBATCH --error=e3b_17_ckpt25_test.err              	# Standard error file
#SBATCH --account=
...
srun python -u run.py --exp-config configs/model_configs/impact_pixel/ppo_impact_pixel_mp3d_test_noisy_e3b.yaml --run-type eval
