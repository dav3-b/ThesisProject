#!/bin/bash
#SBATCH --job-name=occant_ckpt25_test               	# Job name
#SBATCH --partition=					# Specify the partition or queue name
#SBATCH --gres=gpu:1                        		# GPU
#SBATCH --mem=24G                        		# Memory per node (in GB)
#SBATCH --time=24:00:00                 		# Time limit (hh:mm:ss)
#SBATCH --output=occant_ckpt25_acc_test.out             # Standard output file
#SBATCH --error=occant_ckpt25_acc_test.err              # Standard error file
#SBATCH --account=
...
srun python -u run.py --exp-config configs/model_configs/occant/ppo_occant_ar_mp3d_test_noisy.yaml --run-type eval
