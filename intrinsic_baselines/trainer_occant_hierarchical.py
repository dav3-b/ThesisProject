...
from intrinsic_baselines.common.e3b_models_hierarchical import E3BModels
from intrinsic_baselines.common.models import InverseModel, ForwardModel, InverseDynamicsNet, ForwardDynamicsNet
...
@baseline_registry.register_trainer(name="ppo_occant_hierarchical")
class PPOTrainerOccantHierarchical(BaseRLTrainer):
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config, ans_cfg: Config) -> None:
        ...
        #self.reward_models = E3BModels(
        #    clip_param=ppo_cfg.clip_param,
        #    update_epochs=ans_cfg.CURIOSITY.update_epochs,
        #    obs_resized_dim=ans_cfg.obs_resized_dim,
        #    num_concat_obs=ans_cfg.CURIOSITY.num_concat_obs,
        ...
        #    #fwd_model=ForwardDynamicsNet(self.envs.action_spaces[0].n,
        #    #                       hidden_dim=ans_cfg.CURIOSITY.curiosity_hidden_size),
        #    inv_model=InverseModel(self.envs.action_spaces[0].n,
        #                           state_size=ans_cfg.CURIOSITY.curiosity_hidden_size,
        #                           hidden_size=ans_cfg.CURIOSITY.curiosity_hidden_size // 2),
        #    #inv_model=InverseDynamicsNet(self.envs.action_spaces[0].n,
        #    #                       emb_size=ans_cfg.CURIOSITY.curiosity_hidden_size,
        #    #                       hidden_size=ans_cfg.CURIOSITY.curiosity_hidden_size // 2),
        #    impact_hidden_size=ans_cfg.CURIOSITY.curiosity_hidden_size,
        #    paper_encoder=ans_cfg.paper_encoder,
        #    pretrained_paper_encoder=ans_cfg.pretrained_paper_encoder,
        #    pretrained_paper_encoder_weights=ans_cfg.pretrained_paper_encoder_weights,
        #    action_space=self.envs.action_spaces[0],
        #    config=self.config,
        #    #pretrained_visual_encoder=ppo_cfg.pretrained_visual_encoder,
        ...
        #self.inverse_model = self.reward_models.inv_model
        #self.forward_model = self.reward_models.fwd_model
    ...
    def resume_checkpoint(self, path=None):
        ...
        if path is None:
            if len(checkpoints) == 0:
                ...
                #self.forward_model.load_state_dict(ckpt_dict["forward_model"])
                #self.inverse_model.load_state_dict(ckpt_dict["inverse_model"])
    ...
    #def _compute_e3b_rewards(self, features):
    #    u = torch.bmm(features.unsqueeze(1), self.C_inv)
    #    elliptical_bonus = torch.bmm(u, features.unsqueeze(2))
    ...
    #    torch.add(self.C_inv, self.outer_product_buffers, out=self.C_inv)
    #    return elliptical_bonus[:,0] * self.BONUS_COEF
        ...
        for t in range(NUM_LOCAL_STEPS):
            ...
            if t == 0:
                ...
                if hasattr(self.config.RL.PPO,
                    ...
                    map_reward = self._compute_mapbased_global_metric(
                        ground_truth_states, mapper_outputs, state_estimates,
                    )
                    self.wandb_train_stats['train/area_seen'] = map_reward.mean().item()

                    #global_reward_metric = self.episode_custom_rewards.to(device) + (map_reward * 1e-3)
                    #global_reward_metric = self.episode_custom_rewards.to(device) 
                    global_reward_metric = map_reward * 1e-3 
                
                # Update reward for previous global_policy action
                if global_rollouts.current_rollout_step_idx == 0:
                ...
                global_rollouts.insert_rewards(
                ...
                global_rollouts.advance_rollout()
            ...
            if local_rollouts.current_rollout_step_idx > 0:
                local_rollouts.insert_rewards(
            ...
            local_rollouts.advance_rollout()
            ...
            #    
            #    if self.impact_hidden_states is None and ans_cfg.CURIOSITY.use_curiosity_rnn:
            #        self.impact_hidden_states = torch.zeros_like(impact_rollouts.recurrent_hidden_states[0])
            ...
            #        if self.config.RL.PPO.visitation_count == 'e3b':
            #            with torch.no_grad():
            #                #self.elliptical_bonus_buffer.copy_(elliptical_bonus[:, 0])
            #                #e3b_rewards = torch.tensor(self.elliptical_bonus_buffer.squeeze(), dtype=torch.float, device=device)
            #                #e3b_rewards = e3b_rewards.unsqueeze(1)
            #                e3b_rewards = self._compute_e3b_rewards(new_features)
            #        else:
            #            raise ValueError

            #        self.episode_custom_rewards = self.episode_custom_rewards.to(
            #            device) + e3b_rewards.clone().detach()

            #    self.impact_obs = new_imp_obs
            #    del new_imp_obs
            #    
            #    impact_rollouts.insert(
            #        self.impact_obs,
            #        self.impact_hidden_states,
            #        actions,
            #        masks=new_masks,
            #    )
            #    #impact_rollouts.advance_rollout()
    ...
    def _supplementary_rollout_update(
        ...
        if update_option == "local":
            for k, v in local_policy_inputs.items():
                local_rollouts.buffers['observations'][k][local_rollouts.current_rollout_step_idx].copy_(v)
            local_rewards = intrinsic_rewards["local_rewards"].cpu()
            local_rollouts.insert_rewards(
                local_rewards * ppo_cfg.local_reward_scale
            )
            local_masks = local_policy_outputs["local_masks"]
            #local_rollouts.buffers['masks'][local_rollouts.current_rollout_step_idx].copy_(local_masks)
            local_rollouts.insert_last_step(local_policy_inputs, local_masks)
        ...
        if update_option == "global":
            ...
            for k, v in global_policy_inputs.items():
                global_rollouts.buffers['observations'][k][global_rollouts.current_rollout_step_idx].copy_(v)
            if hasattr(self.config.RL.PPO,
                ...
                #global_reward_metric = self.episode_custom_rewards.to(self.device) + (map_reward * 1e-3)
            ...
            #global_rollouts.buffers['masks'][global_rollouts.current_rollout_step_idx].copy_(masks)
            global_rollouts.insert_last_step(global_policy_inputs, masks)
    ...
    def _update_local_agent(self, local_rollouts):
        ...
        with torch.no_grad():
            last_observation = {
                k: v[-1] for k, v in local_rollouts.buffers['observations'].items()
            }
            next_local_value = self.local_actor_critic.get_value(
                last_observation,
                local_rollouts.buffers['recurrent_hidden_states'][-1],
                local_rollouts.buffers['prev_actions'][-1],
                local_rollouts.buffers['masks'][-1],
            ).detach()
    ...
    def _update_global_agent(self, global_rollouts):
        ...
        with torch.no_grad():
            last_observation = {
                ...
                for k, v in global_rollouts.buffers['observations'].items()
            }
            next_global_value = self.global_actor_critic.get_value(
                last_observation,
                None,
                global_rollouts.buffers['prev_actions'][-1].to(self.device),
                global_rollouts.buffers['masks'][-1].to(self.device),
            ).detach()
    ...
    #def _update_reward_model(self, impact_rollouts, flag):

    #    self.reward_models.train()

    #    t_update_model = time.time()
    #    
    #    if self.reward_models.fwd_model is not None:
    #        (
    #            tot_loss,
    #            fwd_loss,
    #            inv_loss
    #        ) = self.reward_models.update(impact_rollouts, self.config.RL.ANS, flag)
    #    else:
    #        inv_loss = self.reward_models.update(impact_rollouts, self.config.RL.ANS, flag)

    #    impact_rollouts.after_update()
    ...
    def _create_global_rollouts(self, ppo_cfg, ans_cfg):
        ...
        global_rollouts = RolloutStorageExtended(
            ...
            #    0 if self.obs_encoder._state_encoder_type == "custom" 
            #    else 1
            #)
        )
    ...
    #def _create_impact_rollouts(self, ppo_cfg, ans_cfg):
    ...
    #                                                 dtype='float32')
    #    
    #    #if not self.reward_models.paper_encoder and self.obs_encoder._state_encoder_type == "custom":
    #    #    num_recur_layers = 0
    #    #elif self.reward_models.paper_encoder and self.reward_models.rnn_type == "none":
    #    #    num_recur_layers = 0
    #    #else:
    #    #    num_recur_layers = self.obs_encoder.num_recurrent_layers
    #    try:
    #        num_recur_layers = self.obs_encoder.num_recurrent_layers
    ...
    def train(self) -> None:
        ...
        #self.impact_obs, impact_rollouts = self._create_impact_rollouts(ppo_cfg, ans_cfg)
        ...
        #impact_rollouts.to(self.device)
        ...
        #statistics_dict["losses"] = defaultdict(
        #    lambda: deque(maxlen=ppo_cfg.loss_stats_window_size)
        #)
        ...
        # Wandb statistics dict
        self.wandb_train_stats = {}
        ...
        for k, v in local_rollouts.buffers["observations"].items():
            mem = v.element_size() * v.nelement() * 1e-9
            print(f"key: {k:<40s}, memory: {mem:>10.4f} GB")
            total_memory_size += mem
        print(f"Total memory: {total_memory_size:>10.4f} GB")
        ...
        #total_memory_size = 0
        #print("================== Impact rollouts ====================")
        #for k, v in impact_rollouts.observations.items():
        #    mem = v.element_size() * v.nelement() * 1e-9
        #    print(f"key: {k:<40s}, memory: {mem:>10.4f} GB")
        #    total_memory_size += mem
        #print(f"Total memory: {total_memory_size:>10.4f} GB")
        ...
        state_estimates = {
            ...
            #).to(self.device),
            "recurrent_hidden_states": torch.zeros(
                self.envs.num_envs, 1, ans_cfg.LOCAL_POLICY.hidden_size
            ).to(self.device),
        ...
        # Insert first observation in impact rollouts
        #for sensor in self.impact_obs:
        #    impact_rollouts.observations[sensor][0].copy_(
        #        self.impact_obs[sensor]
        #    )
        ## Set starting action
        #impact_rollouts.prev_actions[0] = torch.ones_like(
        #    impact_rollouts.prev_actions[0]) * (len(
        ...
        # E3B objects
        #RIDGE = 0.1 # regularization term
        #self.BONUS_COEF = 0.1
        #self.C = torch.stack([torch.eye(self.config.T_MAX) * RIDGE \
        #                        for i in range(self.envs.num_envs)]).to(self.device) # covariace matrix
        #self.C_inv = torch.stack([torch.eye(self.config.T_MAX) * (1./RIDGE) \
        #                            for i in range(self.envs.num_envs)]).to(self.device) # inverse covariace matrix
        #self.outer_product_buffers = torch.zeros(self.envs.num_envs, self.config.T_MAX, self.config.T_MAX).to(self.device)
        #self.elliptical_bonus_buffer = torch.empty(self.envs.num_envs, 1)
        #size = self.config.RL.ANS.CURIOSITY.curiosity_hidden_size
        #self.C_inv = torch.stack([torch.eye(size) * (1./RIDGE) \
        #                            for i in range(self.envs.num_envs)]).to(self.device) # inverse covariace matrix
        #self.outer_product_buffers = torch.zeros(self.envs.num_envs, size, size).to(self.device)
        ...
        for update in tqdm(range(num_updates_start, NUM_GLOBAL_UPDATES)):
            self.update = update
            for i in tqdm(range(NUM_GLOBAL_STEPS)):
                ...
                if UPDATE_LOCAL_FLAG:
                    ...
                    # Sanity check
                    assert local_rollouts.current_rollout_step_idx == local_rollouts.numsteps
                ...
                # -------------------- update inverse models ------------------------
                #if UPDATE_IMPACT_FLAG:

                #    # Sanity check
                #    assert impact_rollouts.step == local_rollouts.numsteps

                #    #if(update % 25 == 0 and i == 0):
                #    #    SAVE_FEATURES = True
                #    #else:
                #    SAVE_FEATURES = False
                ...
                #        delta_pth_time,
                #        update_metrics
                #    ) = self._update_reward_model(impact_rollouts, SAVE_FEATURES)

                #    for k, v in update_metrics.items():
                #        statistics_dict["losses"][k].append(v)
                #    
                #    # reset covariance matrices for finished episodes
                #    #if(i != 0 and i % 5 == 0):
                #    #    for j in range(self.envs.num_envs):
                #    #        self.C_inv[j].copy_(torch.eye(self.C_inv.shape[1]) * (1. / RIDGE))
                
                # -------------------------- log statistics -----------------------
                self.wandb_train_stats['train/update'] = update
                self.logger.info('')
                ...
                for k, v in statistics_dict.items():
                    ...
                    for kp, vp in v.items():
                        if len(vp) > 0 and self._is_master_process():
                            ...
                            self.wandb_train_stats['train/' + k + '/' + kp] = np.mean(vp).item()
                ...
                if update >= 0:
                    ...
                    self.wandb_train_stats['train/frames'] = count_steps * self.config.WORLD_SIZE
                    self.logger.info(
                        "Average window size: {}, average area seen: {:.3f},  {}".format(
                            len(window_episode_stats["count"]),
                            self.wandb_train_stats['train/area_seen'],
                            "  ".join(
                    ...
                    self.wandb_train_stats['train/rewards/local_reward'] = deltas['local_reward']/deltas['count']
                    self.wandb_train_stats['train/rewards/global_reward'] = deltas['global_reward']/deltas['count']
                    self.logger.info("================================================\n")

                # -----------------------wandb log statistics -----------------------
                wandb.log(self.wandb_train_stats)
            ...
            # Sanity check
            assert global_rollouts.current_rollout_step_idx == NUM_GLOBAL_STEPS
            ...
            if episode_step_count[0].item() == self.config.T_EXP:
                ...
                #    self.C_inv[i].copy_(torch.eye(self.C_inv.shape[1]) * (1. / RIDGE))
    ...
    def _eval_checkpoint(
        ...
        if self.config.EVAL.USE_CKPT_CONFIG:
            #del ckpt_dict["config"]["TASK_CONFIG"]["SIMULATOR"]["AGENT_0"]["MASS"]
            #del ckpt_dict["config"]["TASK_CONFIG"]["SIMULATOR"]["AGENT_0"]["LINEAR_ACCELERATION"]
            #del ckpt_dict["config"]["TASK_CONFIG"]["SIMULATOR"]["AGENT_0"]["LINEAR_FRICTION"]
            #del ckpt_dict["config"]["TASK_CONFIG"]["SIMULATOR"]["AGENT_0"]["ANGULAR_ACCELERATION"]
            #del ckpt_dict["config"]["TASK_CONFIG"]["SIMULATOR"]["AGENT_0"]["ANGULAR_FRICTION"]
            #del ckpt_dict["config"]["TASK_CONFIG"]["SIMULATOR"]["AGENT_0"]["COEFFICIENT_OF_RESTITUTION"]
        ...
        #config.TASK_CONFIG.SIMULATOR.HABITAT_SIM_V0.mass = config.TASK_CONFIG.SIMULATOR.AGENT_0.MASS
        #config.TASK_CONFIG.SIMULATOR.HABITAT_SIM_V0.linear_acceleration = config.TASK_CONFIG.SIMULATOR.AGENT_0.LINEAR_ACCELERATION
        #config.TASK_CONFIG.SIMULATOR.HABITAT_SIM_V0.linear_friction = config.TASK_CONFIG.SIMULATOR.AGENT_0.LINEAR_FRICTION
        #config.TASK_CONFIG.SIMULATOR.HABITAT_SIM_V0.angular_acceleration = config.TASK_CONFIG.SIMULATOR.AGENT_0.ANGULAR_ACCELERATION
        #config.TASK_CONFIG.SIMULATOR.HABITAT_SIM_V0.angular_friction = config.TASK_CONFIG.SIMULATOR.AGENT_0.ANGULAR_FRICTION
        #config.TASK_CONFIG.SIMULATOR.HABITAT_SIM_V0.coefficient_of_restitution = config.TASK_CONFIG.SIMULATOR.AGENT_0.COEFFICIENT_OF_RESTITUTION
        config.freeze()
        ...
        for k, v in mapping_metrics.items():
            ...
            for kp in sorted(list(metric_all_times.keys())):
                ...
                if(k == "area_seen"):
                    wandb_eval_stats['eval/mapping_metrics/' + k + '_' + str(kp)] = vp
            writer.add_scalar(
                f"mapping_evaluation/{k}", v.get_last_metric(), num_frames_per_process,
            )
        
        # -----------------------wandb log statistics -----------------------
        wandb.log(wandb_eval_stats)
