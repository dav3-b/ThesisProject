...
class FwdStateEncoder(nn.Module):

    def __init__(self, state_size=512, hidden_size=512):
        super(FwdStateEncoder, self).__init__()
...
class ForwardDynamicsNet(nn.Module):
    def __init__(self, num_actions, hidden_dim=1024):
        ...
        self.forward_dynamics = nn.Sequential(
            init_(nn.Linear(hidden_dim + self.n_actions, hidden_dim)), 
            nn.ReLU(), 
        )
    ...
    def forward(self, state_embedding, action_one_hot):
...
class InverseDynamicsNet(nn.Module):
    def __init__(self, num_actions, emb_size=512, hidden_size=256, p_dropout=0.0):
        ...
        self.inverse_dynamics = nn.Sequential(
            init_(nn.Linear(2 * emb_size, hidden_size)), 
            nn.ReLU(),
        ...
        init_ = lambda m: init(m, nn.init.orthogonal_, 
            lambda x: nn.init.constant_(x, 0))
        self.id_out = init_(nn.Linear(hidden_size, self.n_actions))
...
def resnet18(in_channels, pretrained=False, progress=True, **kwargs):
    ...
    """
    return _resnet('resnet18', in_channels, BasicBlock, [2, 2, 2, 2], pretrained, progress,
                   **kwargs)
